...
In each of the previous examples, they is a certain pattern. In order to define a single interaction, it requires the need to know which SVG element to target, what type of interaction or event is to be attached to that element, and how to define what happens when an interaction occurs. This idea can be broken down into 5 simple steps:
  ...
  ```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:main-idea} grid, gridSVG and DOM in the process"}
...
```{r, fig.cap = "\\label{fig:bp} Producing a box plot in R"}
listElements(bw)
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-1} Box plot with hover interaction"}
...
We see that the box plot and the scatter plot we drew in R are now on the same web page (\autoref{fig:int-bp-2}).

```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-2} Boxplot and scatter plot on the same web page"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-3} Click on box plot to light up points on scatter plot"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-4} All three plots on the same web page"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-5} A single click on the box plot links the density and scatterplot together"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-slider-1} Plot with slider"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-slider-2} Plot with slider that controls the smoothness of the trend line"}
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-slider-3} Plot that has a selection box feature that draws a separate smoother"}
...
A useful feature of interactr is its compatibility with other R graphing systems including \textsf{lattice}, \textsf{graphics} (also known as base R plots), and \textsf{ggplot2}.
...
grid.ls()
```
```{r, echo = FALSE, fig.cap = "\\label{fig:gg-convert} An example of a graphics plot that is converted into a grid plot using gridGraphics"}
library(grid)
plot(1:10, 1:10)
grid.ls()
gridGraphics::grid.echo()
...
```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-graphics} Box plot example replicated using a graphics plot"}
knitr::include_graphics('./fig/int-bp-graphics.png')
```

The same interaction has been achieved in \autoref{int-bp-graphics}. This shows that there is potential for customising interactions onto graphics plots. The process is the same, except for an additional step to convert a \textsf{graphics} plot into a \textsf{grid} type plot.

### ggplot2

\textsf{ggplot2} [@ggplot01] is a popular plotting system in R based upon the "Grammar of Graphics". It is built upon the \textsf{grid} graphics system, which makes it compatible with \textsf{gridSVG}.
