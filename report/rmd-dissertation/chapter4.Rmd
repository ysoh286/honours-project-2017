...
In this step, we have sent the plot we drew in R to a new web page. We can also attach an interaction to the `box` element we identified earlier to attach the hover interaction to. On the page (\autoref{fig:int-bp-1}), we see that when the user hovers over the box, the box turns red.

```{r, echo = FALSE, out.width = "70%", fig.cap = "\\label{fig:int-bp-1} Boxplot with hover interaction"}
...
Here, we see that the box plot and the scatter plot we drew in R are now on the same web page (\autoref{fig:int-bp-2}).
...
Here, we insert the function name to run when a 'click' is done. Next, we append this defined interaction to the box element, so that when we click on the box, the points in the scatterplot that lie within that range should light up in red. This is shown in \autoref{fig:int-bp-3}.
...
grid.ls()
```
```{r, echo = FALSE, fig.cap = "\\label{fig:gg-convert} An eample of a graphics plot that is converted into a grid plot using gridGraphics"}
library(grid)
plot(1:10, 1:10)
grid.ls()
gridGraphics::grid.echo()
