...
The motivation behind this research comes from the idea of creating interactive
 plots with \textsf{iNZight}, a data visualisation software from the University of Auckland.

An overview of modern web tools were investigated including \textsf{plotly}, \textsf{ggvis}, \textsf{shiny} and \textsf{animint}.  It is easy to achieve certain interactions, but hard to extend beyond their capabilities without a deeper understanding of these packages and lower level coding. This makes it inaccessible to the majority of users.  Furthermore, many online systems have a
...
A different approach was taken by investigating lower level tools, specifically \textsf{gridSVG} and \textsf{DOM}. These tools are extensible, however, to use them effectively requires a
knowledge about how the grid system works with gridSVG and web technologies including the Document Object Model. This presents a steeper learning curve than using \textsf{plotly} and \textsf{ggvis}, and consequently a trade off - to achieve custom interactions, a user would be required to know how to link all these tools together, where as other tools are easier to use but cannot be extended further.
...
\textsf{graphics}, \textsf{lattice}, and \textsf{ggplot2}.

The \textsf{interactr} package stands out as it brings interactivity to plots that
...
of these tools are developing over time. It is possible that the \textsf{interactr}
package may become a solution for allowing users to control interactions more
easily on plots in R and thus for \textsf{iNZight}, but requires more attention and
