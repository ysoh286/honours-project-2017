...
```{r, echo = FALSE, fig.cap = "Figure 4.2: Boxplot (left) that turns red when hovered (right)"}
...
We have inserted the function name to run when a 'click' is performed (line 1). Next in the second line of code, we have appended this interaction to the box element, so that when we click on the box, the points in the scatterplot that lie within that range should light up in red. This is shown in Figure 4.5.
...
The same interaction has been achieved in Figure 4.12. This shows that there is potential for customising interactions onto graphics plots. The process is the same, except for an additional step to convert a **graphics** plot into a **grid** type plot.
...
Next, we can use the same function [`highlightPoints`](#highlightPoints) defined before sending these interactions to the browser  (Figure 4.13).
