...
```
This line of code (the `draw` function) both links the interaction we defined before to the box element and sends the plot across to a new web page. We see that when the user hovers over the box, the box turns red as seen in [Figure 4.3](#fig-4.3).

<a id = "fig-4.3"> </a>

```{r, echo = FALSE, fig.cap = "Figure 4.3: Boxplot (left) that turns red when hovered (right)"}
...
We see that the box plot and the scatter plot we drew in R are now on the same web page ([Figure 4.4](#fig-4.4)).

<a id = "fig-4.4"> </a>

```{r, echo = FALSE, fig.cap = "Figure 4.4: Box plot and scatter plot on the same web page"}
...
We have inserted the function name to run when a 'click' is performed (line 1). Next in the second line of code, we have appended this interaction to the box element, so that when we click on the box, the points in the scatterplot that lie within that range should light up in red. This is shown in [Figure 4.5](#fig-4.5).

<a id  = "fig-4.5"> </a>

```{r, echo = FALSE, fig.cap = "Figure 4.5: Click on box plot to light up points on scatter plot"}
knitr::include_graphics('./fig/int-bp-3.png')
```
**Click [here](http://104.236.190.93:3838/YuHan/link-box-lattice/) to view the example in Figure 4.5.**
...
```{r, echo = FALSE, fig.cap = "Figure 4.6: All three plots on the same web page"}
knitr::include_graphics('./fig/int-bp-4.png')
```
[Figure 4.6](#fig-4.6) shows all three plots on the same web page.
...
When the user now clicks on the box plot, it lights up the points and the density that lie within that range as seen in [Figure 4.7](#fig-4.7).

<a id = "fig-4.7"> </a>

```{r, echo = FALSE, fig.cap = "Figure 4.7: A single click on the box plot links the density and scatterplot together"}
knitr::include_graphics('./fig/int-bp-5.png')
```

**Click [here](http://104.236.190.93:3838/YuHan/box-density/) to view the example demonstrated in Figure 4.7.**
...
Next, we add a slider to the page. This has not been linked up to any elements yet ([Figure 4.8](#fig-4.8)).
...
```{r, echo = FALSE, fig.cap = "Figure 4.8: Plot with slider"}
...
```
Finally, we can link the `sliderValue` function back to the slider such that when the slider moves, the trend line will be updated based upon the value of the slider as seen in [Figure 4.9](#fig-4.9).
...
```{r, echo = FALSE, fig.cap = "Figure 4.9: Plot with slider that controls the smoothness of the trend line"}
...
```{r, echo = FALSE, fig.cap = "Figure 4.10: Plot that has a selection box feature that draws a separate smoother"}
knitr::include_graphics('./fig/int-slider-3.png')
```
**Click [here](http://104.236.190.93:3838/YuHan/link-trendline/) to view the entire example in Figure 4.10, which controls a trend line with a slider and a drag-box to create a new smoother.**

The user now can draw a selection box over a set of points, and a new smoother should render on the page based upon these points (shown in [Figure 4.10](#fig-4.10)).
...
The examples discussed so far with **interactr** have been done with **lattice** plots. However, it is possible to achieve the same with other plotting systems. To demonstrate, we have taken the box plot example in [Figure 4.5](#fig-4.5) and replicated it using **graphics** and **ggplot2**.
...
```{r, echo = FALSE, fig.cap = c("", "Figure 4.11: An example of a graphics plot that is converted into a grid plot using gridGraphics")}
library(grid)
plot(1:10, 1:10)
grid.ls()
gridGraphics::grid.echo()
...
```{r, echo = FALSE, fig.cap = "Figure 4.12: box plot example replicated using a graphics plot"}
knitr::include_graphics('./fig/int-bp-graphics.png')
```
**Click [here](http://104.236.190.93:3838/YuHan/link-box-graphics/) to view the example shown in Figure 4.12.**

The same interaction has been achieved in [Figure 4.12](#fig-4.12). This shows that there is potential for customising interactions onto graphics plots. The process is the same, except for an additional step to convert a **graphics** plot into a **grid** type plot.
...
Next, we can use the same function [`highlightPoints`](#highlightPoints) defined before sending these interactions to the browser  ([Figure 4.13](#fig-4.13)).
...
```{r, echo = FALSE, fig.cap = "Figure 4.13: box plot example replicated using ggplot2"}
knitr::include_graphics('./fig/int-bp-ggplot2.png')
```
**Click [here](http://104.236.190.93:3838/YuHan/link-box-ggplot2/) to view the example shown in Figure 4.13.**

This demonstrates that there is a possible way of achieving interactions with **ggplot2**. Because this is a simplistic example, it may become more complex when we try to achieve more sophisticated interactions.
