...
To illustrate, we create a simple example linking between gender and highest qualification to identify if there is a difference in income from the `nzincome` data set (Figure 2.13). When we click on the bars of the plot (left), we can subset the data by gender, while we can further subset the groups into qualification by selecting the legend on the scatter plot on the right. Overall,
