...
However, if it is based upon a defined plotting system without a consisten naming scheme like ggplot2, then there is a need for deducing which elements correspond to which part of the plot. 
...
In comparison to ggvis, the `interactr` package may not be able to win against its simpler API. `ggvis` is one of the few packages that uses the `shiny` infrastructure but does not redraw whenever an input changes.
However, 
