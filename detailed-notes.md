...
# DEADLINE : TUESDAY 31st OCTOBER, 2017  12PM NZT
...
CHAPTER 1: [Scheduled for Week 10]
- Include a glossary of web tech terms rather than try writing in para?
- Do another quick lit review?
- background information...zzzz
- ON PLOT + OFF PLOT INTERACTIONS - WHAT ARE THEY?
- Include an overview for the different graphic systems in R?

CHAPTER 2:
- ~~animint package: review + assess limitations~~
- ~~Write animint section~~
- ~~Write plotly section (a quick note on dynamic aggregates + preventing redraws in shiny)~~
- Add in ~~missing figures + shiny app~~ + animint example
- ~~Include a summary table at the end of this chapter~~
- ~~rephrase htmlwidgets (mention that this is what is generated by these packages, not that they are 'these packages')~~
- once chapter 2 is finalized, email Paul which shiny apps to put up (host 3-5 on the public server?)

CHAPTER 3:
- ~~send 1st draft of Chapter 3 up for review~~
- ~~Mention that there's some new content passing through~~
- REWRITE DOM SECTION (how would you explain the DOM to a code-free friend?)
- Look up on shiny's new asynchronous stuff (that could potentially suggest a speed up + more responsive...? something about futures and promises?) [ is the explanation for making things asynchronous relevant for the reader in terms of driving interactive graphics?]

CHAPTER 4:
- ~~break code into smaller chunks~~
- ~~get rid of redundant/repetitive stuff~~
- add in appropriate titles for the box/scatters
- optional: could add more examples/'applications' if there's time (which there isn't :()

CHAPTER 5: [COMPLETE BY END OF WEEK 9!]
- Explain similarities and differences to existing tools
- Limitations of current implementation
- Examples for comparison?

LAST TODOS BEFORE SUBMISSION:
- REFERENCES
- Add on acknowledgements
- Review conclusion/exec-sum
- Write an abstract
- LaTeX format for pdf, HTML format for website
...
- Try out the xaringan package for presentation slides (... is it compulsory to do a talk?)


**new resources:**
- Plotly 4.7.1 [blog post](https://moderndata.plot.ly/plotly-4-7-1-now-on-cran/) that shows prevention of redrawing using 'plotly proxy' objects (inspired by Leaflet and DT?)
- Plotly's achieved [dynamic aggregation]() - which means you can link other kinds of plots (not just scatterplots/row-observational data!)
...
## TO DO LIST for interactr (after honours)

...unless I get bored and abandon it after this project is finished.
...
- try communicating between two webpages? (could you open in a new window?)
...
- instead of using grid tags, use SVG tags instead for consistency with cases that do not use gridSVG? (use getSVGMappings() to get correct id/selectors - in a case of gridSVG, the tags are not the same as those generated by grid.)
  - this needs to be revised for adding interactions onto multiple elements (or elements that do not end with a '1', or are part of a group( ie. like the last point of a group of points))
