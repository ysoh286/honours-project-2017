...
## REPORT STATUS: COMPLETE AND SUBMITTED.
...
- ~~Able to attach a single hover interaction to more than 1 element
(e.g. if it's defined in grid as 1 element but corresponds to a group, such as
a set of bars)~~


**Issues/Bugs/TODO/TOFIX:**

Finding a kazillion things wrong with interactr... :/

- Come up with a better structure for the package (ie possibilities of coming up with a way of identifying plot class that can run through all functions)
- FIX SCOPING ISSUES
- VALIDATION/TEST STOPS/ERROR CHECKS (read up on what is unit testing)
...
- ~~instead of using grid tags, use SVG tags instead for consistency with cases that do not use gridSVG? (use getSVGMappings() to get correct id/selectors - in a case of gridSVG, the tags are not the same as those generated by grid.)~~ solved using gridSVGMappings
